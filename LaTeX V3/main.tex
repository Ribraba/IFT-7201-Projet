...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.5\linewidth]{../figures/training_hard.png}
...
\begin{figure}[ht]
  ...
  \begin{subfigure}[t]{0.49\linewidth}
    ...
    \caption{Taux de succès à l'évaluation (politique greedy, 500 épisodes). Les échecs sont exclusivement des \textit{timeouts} — le taux de chutes est nul.}
    \label{fig:overview}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.49\linewidth}
...
Le Tableau~\ref{tab:eval} synthétise les indicateurs d'évaluation. Sur le cas Difficile, l'écart de boucles est frappant : Q-learning accumule $15{,}3\% \pm 7{,}7\%$ de \textit{timeouts} contre seulement $3{,}1\% \pm 1{,}7\%$ pour SARSA. SARSA réduit également la proximité aux trous ($27{,}4\%$ contre $32{,}8\%$) et emprunte davantage le chemin sûr identifié par Dijkstra~\citep{dijkstra1959} ($66{,}3\% \pm 2{,}5\%$ contre $63{,}7\% \pm 4{,}2\%$), avec une variance moindre confirmant une plus grande cohérence.

\begin{table}[ht]
  \centering
  \caption{Indicateurs d'évaluation finale (politique greedy, 500 épisodes, moyenne $\pm$ écart-type, 10 répétitions). Chemin sûr : overlap $\geq 50\%$ avec le chemin Dijkstra (cas Difficile uniquement).}
  \label{tab:eval}
  \small
  \begin{tabular}{llccc}
    \toprule
...
Ces résultats convergent : Q-learning converge plus rapidement mais adopte une politique moins stable en environnement stochastique à obstacles groupés, tandis que SARSA réduit les boucles, la proximité aux trous et préfère davantage le chemin sûr. Ces limitations motivent l'introduction d'un mécanisme de sécurité explicite dans le rapport final.
