...
Un épisode se termine lorsque l'agent atteint l'objectif \texttt{G}, tombe dans un trou \texttt{H}, ou dépasse 200 pas de temps. La fonction de récompense est mise en forme (\textit{reward shaping}) pour densifier le signal d'apprentissage et rendre les comportements indésirables plus visibles empiriquement~\citep{ng1999} : chute ($r = -1{,}0$), objectif ($r = +1{,}0$), collision avec un mur ($r = -0{,}1$), déplacement ordinaire ($r = 0{,}0$).
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{../../figures/hard_shield_tradeoff.png}
  \caption{Cas \textbf{Difficile} : effet du shield au déploiement. Le Q-learning blindé est entraîné de manière identique dans les deux cas ; seule la politique d'évaluation change selon que le shield est maintenu ou non à l'inférence. Le shield actif élimine les chutes ($0{,}0\%$) mais transfère une partie de l'échec vers les boucles ($45{,}2\%$).}
  \label{fig:hard_shield_tradeoff}
\end{figure}

La Figure~\ref{fig:hard_shield_tradeoff} complète les vues d'ensemble sans les dupliquer : sur \textit{Facile} et \textit{Moyen}, activer le shield à l'inférence ne modifie pratiquement pas les résultats, alors que sur \textit{Difficile} l'effet devient structurel. Avec la même table $Q$, le Q-learning blindé passe de $19{,}9\%$ de succès et $75{,}2\%$ de chutes sans shield à $54{,}8\%$ de succès, $0{,}0\%$ de chutes et $45{,}2\%$ de boucles avec shield. Ce résultat montre que le mécanisme de sécurité n'améliore pas seulement l'exploration ; il modifie aussi le compromis sécurité/performance lorsqu'il reste actif au déploiement.
...
\textbf{Liens avec la littérature :} sur le cas Difficile, SARSA emprunte le détour sûr plus souvent que Q-learning ($70{,}4\%$ contre $60{,}1\%$), confirmant le phénomène du \textit{cliff walking} prédit par la théorie~\citep[chap.~6]{sutton2018} : la mise à jour \textit{on-policy} intègre naturellement le coût de l'exploration dans les estimations de valeur. La réduction des chutes par le shield pendant l'entraînement est cohérente avec le principe Fear Field~\citep{Fearfield2025}. Cependant, l'implémentation originale maintient le shield au déploiement, alors que notre protocole principal l'évalue d'abord en politique greedy sans shield. La Figure~\ref{fig:hard_shield_tradeoff} montre que cette distinction de protocole change qualitativement l'interprétation du résultat sur \textit{Difficile}.

\textbf{Explication des résultats :} sur \textit{Facile} et \textit{Moyen}, les trois stratégies convergent vers des politiques équivalentes : la topologie reste simple et la politique greedy finale évite déjà les trous, avec ou sans shield au déploiement. Le cas \textit{Difficile} est d'une nature différente. D'abord, la contrainte $\mathcal{A}_{\text{safe}}(s)$ retire de l'exploration les actions qui mèneraient dans un trou avec probabilité non nulle ; leurs Q-valeurs restent donc peu ou pas informées. Ensuite, Q-learning demeure \textit{off-policy} : la mise à jour utilise $\max_{a'} Q(s',a')$, y compris pour des actions rarement ou jamais jouées à cause du masquage. On obtient ainsi une table $Q$ partiellement incohérente près de la barrière centrale, où certaines actions non testées conservent une valeur artificiellement compétitive. Enfin, pour plusieurs états adjacents à la barrière, $\mathcal{A}_{\text{safe}}(s)=\emptyset$ à cause de la stochasticité : le repli autorise alors toutes les actions exactement dans la zone la plus risquée, ce qui détériore encore l'estimation locale. Sans shield à l'évaluation, la politique greedy peut donc sélectionner des actions dont la dangerosité n'a jamais été réellement internalisée, d'où les $75{,}2\%$ de chutes observés.

Le comportement \textit{avec} shield au déploiement confirme cette lecture. En maintenant le filtrage à l'inférence, les chutes tombent à $0{,}0\%$, ce qui montre que le mécanisme de sécurité remplit bien son objectif de court terme : empêcher l'entrée dans les états dangereux. En revanche, le taux de boucles monte à $45{,}2\%$ et le succès plafonne à $54{,}8\%$. Le shield ne corrige donc pas la qualité de la table $Q$ ; il transforme une partie des échecs par chute en échecs par incapacité à progresser. Autrement dit, le modèle tabulaire blindé obtenu ici garantit mieux la sécurité \emph{pas à pas}, mais pas l'efficacité globale de la navigation.

\textbf{Réflexion :} SARSA offre le meilleur rapport qualité-sécurité sans mécanisme dédié : sa mise à jour \textit{on-policy} produit la meilleure politique finale sur \textit{Difficile} ($81{,}2\%$ de succès, $25{,}9\%$ de proximité aux trous) parce qu'elle internalise naturellement le coût de l'exploration. Le shield tabulaire, lui, est computationnellement léger et très efficace pour interdire immédiatement les transitions dangereuses, mais il introduit une tension structurelle entre \emph{sûreté locale} et \emph{progrès global}. Plus le filtrage est conservateur dans un environnement glissant, plus il protège contre les trous ; mais plus il risque aussi d'enfermer l'agent dans des comportements prudents mais improductifs. Cette tension explique pourquoi la version \emph{avec shield} réduit les chutes à zéro sans rattraper les performances de SARSA.

D'un point de vue pratique, cette expérience souligne aussi une limite importante de notre simplification : le shield repose ici sur la table exacte $P(s'|s,a)$ fournie par Gymnasium. Dans un système réel, cette connaissance devrait être apprise ou approximée ; les erreurs de modèle déplaceraient encore le compromis observé. Deux prolongements naturels seraient donc (1) de maintenir explicitement le shield au déploiement lorsqu'une garantie de sûreté prime sur le taux de succès, et (2) de remplacer la contrainte stricte ($p>0$) par un seuil probabiliste ($p>\theta$) afin de réduire la sur-conservativité dans les zones où la glissade rend toute action localement risquée.

\textbf{Message général au client :} réduire les situations dangereuses pendant l'apprentissage est atteint par les trois stratégies sur les cas simples, mais l'environnement \textit{Difficile} révèle qu'il faut distinguer \emph{apprendre prudemment} de \emph{agir sûrement au déploiement}. Le Q-learning blindé simplifié n'est pas une meilleure politique finale en soi : sans shield à l'inférence, il chute massivement ; avec shield, il devient sûr mais plus souvent bloqué. La recommandation est donc d'utiliser \textbf{SARSA} comme base robuste pour des environnements stochastiques à obstacles groupés, et de réserver un shield maintenu au déploiement aux contextes où l'élimination des chutes est prioritaire même au prix d'un plus grand nombre de boucles.
